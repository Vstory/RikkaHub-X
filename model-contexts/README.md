...
```jsonc
{
  ...
  "updatedAt": "2026-09-10T23:30:30+08:00",  // 数据核验时刻(ISO-8601,须带秒与时区偏移)
...
- **删模型**:对应规则/条目直接移除;
- 改完顺手把 `updatedAt` 更新为**当前时刻**。

### `updatedAt` 的格式要求(会被测试强制)

必须是**带秒与时区偏移的 ISO-8601**,例如 `2026-09-10T23:30:30+08:00`:

- **必须带秒** —— App 设置页要显示成 `数据日期:2026-09-10 23:30:30`,只到日的旧格式撑不起时分秒;
- **必须带时区偏移** —— 否则不同时区的人看到的时刻含义不同;App 会按本机时区换算后显示;
- 秒以下精度(毫秒)可有可无,App 能解析。

**该字段是 App 排序数据新旧的唯一依据,必须可信。** App 会**拒整表**当它:
缺失、乱写、或超前当前时间超过 24 小时(见下)。

### 为什么不允许写到未来

App 从多个源取数据后按 `updatedAt` 选最新的一版。若某个源给出一个远在未来的时间戳
(手滑把年份写成 `2062`,或源被篡改),它会**永远胜出** —— 后续任何正常更新都比不过它,
表被永久钉死,而界面看不出任何异常。

故 App 拒绝超前当前时间超过 24 小时的 `updatedAt`(24 小时是为了吸收设备时钟偏移;
联网设备的系统时间由 NTP 校准,现实偏移远小于此)。

注意这**只是粗错闸门,不是防篡改手段**:能改写响应体的中间人完全可以把时间戳改到
"当前时间附近"。真正的完整性要靠签名校验。

### 数据来源

App 只从 **GitHub 官方**取这份文件:

| 源 | 说明 |
|---|---|
| `api.github.com`(首选) | 不经 CDN 缓存,推完 main 立刻是新内容;未认证有速率限制(60 次/时),而刷新以小时计,足够 |
| `raw.githubusercontent.com`(备份) | 走 CDN(`max-age=300`),实测可能滞后数十分钟,用于备份与交叉印证 |

曾经并列过 `raw.githack.com` / `cdn.statically.io` / `fastly.jsdelivr.net` 三个第三方代理,
已全部移除 —— 它们不属 GitHub 的信任边界却经手数据,且都会改写响应体(实测 jsDelivr 对
超限仓库返回自写的错误文案却带 200),而 App 侧没有签名校验,分辨不出内容是否被改过;
每次刷新还会把用户的 IP、UA 同时交给这些第三方。

**若将来要加第三方镜像做冗余,前提是先做签名校验**(公钥内置 App、私钥只在 CI),
否则镜像既是篡改入口,也是"用未来时间戳把表钉死"的入口。

测试侧:`ContextWindowsDataInvariantTest` 校验格式与语义,
`ContextWindowUpdatedAtGuardTest` 覆盖闸门本身(含"仓库数据文件必须过闸")。

### App 侧显示的两个时间

| 显示 | 来源 | 含义 |
|---|---|---|
| **数据日期** | 表内 `updatedAt` | 这份数据是谁、什么时候核验的(远端权威时刻) |
| **更新时间** | 本机缓存文件时间戳 | **本机**最后一次成功拉取并落盘的时刻,跨重启保留 |

两者刻意不同:前者说明数据的新旧,后者说明**你手上这份**的新旧。若长期不联网,
「数据日期」不变、「更新时间」也停在上次成功拉取的时刻 —— 一眼就能看出表是旧的。
