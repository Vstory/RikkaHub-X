 ...
 *
 * 它们把「会话的属性」**冗余存进了每一行**，而这两者与消息内容无关：
 *
 * | 列 | 何时变 | 后果 |
 * |---|---|---|
 * | `title` | 用户重命名（罕见） | 若要求每行同步，重命名即全量重写 |
 * | **`update_at`** | **每次保存都变** | **若要求每行同步，就等于每次保存全量重写 —— 增量维护根本不可能** |
 *
 * 故本次把**查询改为 `JOIN conversationentity`** 取这两个值（那里才是权威真源）。
 * 冗余列不再被任何查询读取，写入时仍填上「写入那一刻的值」——
 * 因此它们**可能过期，但不会形状错误**；**任何新查询都不要读它们**。
 *
 * 这两列删不掉：FTS5 是虚拟表，没有 `ALTER TABLE ... DROP COLUMN`，
 * 要删只能重建整张表（= 对全部消息重新分词一次，重型迁移）。**收益不抵风险，故留着不用。**
 */
