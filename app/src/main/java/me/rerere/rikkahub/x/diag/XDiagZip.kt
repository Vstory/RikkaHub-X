...
object XDiagZip {
    ...
    internal fun manifest(
        ...
        appendLine()
        appendLine("  This bundle holds ONE session (the most recent one) plus the cross-session")
        appendLine("  files listed above. A session starts each time the recording switch is turned")
        appendLine("  on, and older sessions are deleted automatically once enough of them pile up")
        appendLine("  (see the diagnostic page) -- so no earlier session can be in here, even if you")
        appendLine("  expected to find one.")
        appendLine()
