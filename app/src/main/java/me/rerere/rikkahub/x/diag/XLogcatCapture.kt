...
object XLogcatCapture {
     ...
     *
     * 停止之后仍要能取到:诊断页那一行体积显示要在开关关掉后**继续显示上一次录了多大**
     * (否则关掉开关的瞬间数字就归零,看起来像「刚才那段被清了」)。会话目录名带时间戳,
     * 故按名字取最大即最近一次。
     *
     * ⚠️ 它**不再服务于「导出应用日志」** —— 那个单独导出 2026-09-13 已随导出收敛移除
     * (内容是压缩包里的一个文件)。若哪天有人想拿它做别的导出,先读诊断页顶部那段
     * 「为什么导出只有一个动作」。
     */
